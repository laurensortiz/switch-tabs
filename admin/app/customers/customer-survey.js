...
angular.module('switchTabsAppAdmin')
  .controller('customerCtrl', ['$scope', '$timeout', '$filter', '$location', '$window', '$rootScope', '$routeParams', 'customers', 'breadcrumbs', function ($scope, $timeout, $filter, $location, $window, $rootScope, $routeParams, customers, breadcrumbs) {
    ...
    $scope.showCustomerDetail = function(customerID) {
      ...
      customers.getCustomerInfo(customerID).then( function (customer) {
        ...
        //TODO: Include in the breadcrumb the name of te customer

        breadcrumbs.options = { 'Cliente': 'Cliente ' + customer.name };
      });
