...
angular.module('switchTabsAppPublic')
  .controller('surveyCtrl', function ($scope, $location, $timeout, $filter, $rootScope, $routeParams, survey, customer) {
    ...
    //Get Questions
    survey.getSurveyByCustomer($scope.customer).then(function(surveyID){

     // surveyID = 1;

      survey.getSurveyQuestions(surveyID).then(function (questions) {
    ...
    $scope.saveAnswers = function (idQuestion, answer) {
      ...
      if($scope.currentStep ===  $scope.numberOfQuestions + 1){
        ...
        survey.saveAnswers($scope.saveAnswer).then(function(data){
          resetSurvey(3000);
        });
      }
    }
